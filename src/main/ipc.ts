...
const revisionGetCurrentRequestSchema = z.object({
  entityType: revisionEntityTypeSchema,
  entityId: z.string().trim().min(1),
});

const revisionCreateRequestSchema = revisionGetCurrentRequestSchema.extend({
  label: z.string().trim().max(160).optional(),
});

const revisionListRequestSchema = revisionGetCurrentRequestSchema;

const revisionRestoreRequestSchema = z.object({
  revisionId: z.string().trim().min(1),
});

const entityRevisionResponseSchema = z.object({
  id: z.string(),
  projectId: z.string(),
  entityType: revisionEntityTypeSchema,
  entityId: z.string(),
...
const entityRevisionCurrentResponseSchema = z.object({
  entityType: revisionEntityTypeSchema,
  entityId: z.string(),
  title: z.string(),
  subtitle: z.string(),
...
type RevisionEntityType = z.infer<typeof revisionEntityTypeSchema>;

interface RevisionSnapshotContent {
...
function createAutomaticRevision(
  repository: Repository,
  projectId: string,
  entityType: RevisionEntityType,
  entityId: string,
...
function restoreRevisionSnapshot(
  repository: Repository,
  projectId: string,
...
export function registerIpcHandlers(ipcMain: IpcMain, sessionManager: ProjectSessionManager): void {
  ...
  ipcMain.handle(IPC_CHANNELS.characterUpdateCard, async (_event, payload: unknown) => {
    ...
    createAutomaticRevision(repository, projectId, 'character', request.id);
  ...
  ipcMain.handle(IPC_CHANNELS.locationUpdateCard, async (_event, payload: unknown) => {
    ...
    createAutomaticRevision(repository, projectId, 'location', request.id);
  ...
  ipcMain.handle(IPC_CHANNELS.sceneUpdateCard, async (_event, payload: unknown) => {
    ...
    createAutomaticRevision(repository, projectId, 'scene', request.id);
  ...
  ipcMain.handle(IPC_CHANNELS.revisionCreate, (_event, payload: unknown) => {
    ...
    const revision = repository.createEntityRevision(projectId, {
      entityType: request.entityType,
      entityId: request.entityId,
      label: request.label || 'Versione manuale',
      reason: 'manual',
      snapshotJson: content.snapshotJson,
      textContent: content.textContent,
    });
  ...
  ipcMain.handle(IPC_CHANNELS.revisionList, (_event, payload: unknown) => {
    ...
    buildRevisionContent(repository, projectId, request.entityType, request.entityId);
    const revisions = repository.listEntityRevisions(
      projectId,
      request.entityType,
      request.entityId,
    );
  ...
  ipcMain.handle(IPC_CHANNELS.revisionRestore, async (_event, payload: unknown) => {
    ...
    const revision = repository.getEntityRevisionById(request.revisionId);
    ...
    await syncProjectWikiSourcesBestEffort(sessionManager);
    const current = buildRevisionContent(
      repository,
      projectId,
      revision.entityType,
      revision.entityId,
